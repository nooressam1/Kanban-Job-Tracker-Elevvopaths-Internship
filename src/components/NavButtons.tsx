...
export default function NavButton({ isActive, label, onClick, isCollapsed, ...props }: NavButtonButtonProps) {
    ...
    return (
        <button type="button"
            onClick={onClick} className={`w-full  flex ${isCollapsed ? "px-2" : "px-4"} py-2.5 justify-start items-center transition-all duration-200 overflow-hidden rounded-md text-left text-sm font-medium transition-all ${isActive
